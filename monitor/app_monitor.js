...
global.fs 				= require('fs');
						  ...
						  fs.writeFileSync("app_monitor.pid", process.pid);
...
app.route('/')
   .get(function(req, res) {
    res.render('index', { title: '微影院前端node集群监控界面', message: 'Hello there!'});
});
...
sock.bind(3091);
console.log('axon pull bind on port %d', 3091);

sock.on('message', function(metrics){
  console.log(metrics);
  var host_name = metrics.my_name;

  g_metrics[host_name] = metrics;
});
